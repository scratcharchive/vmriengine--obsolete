...
VmriGpuInterface::VmriGpuInterface() {
	d=new VmriGpuInterfacePrivate;
	d->m_T1=100;
	d->m_T2=40;
	d->m_gamma=42.57;
}
...
void VmriGpuInterface::setT1(double T1) {
	d->m_T1=T1;
}
void VmriGpuInterface::setT2(double T2) {
	d->m_T2=T2;
}

void VmriGpuInterface::setIsochromats(const QList<double> &x,const QList<double> &y,const QList<double> &z,const QList<double> &f,const QList<double> &density) {
...
void VmriGpuInterface::addRFWaveform(const QList<double> &data_real,const QList<double> &data_imag,double dt) {
	RF_operator XX;
...
void VmriGpuInterface::initialize() {
}

void VmriGpuInterface::evolve(double A[3],double t,double E1,double E2) {
...
Matrix44 VmriGpuInterfacePrivate::get_RF_operator_matrix(int waveform_index,double dtheta) {
...
void VmriGpuInterface::excite(int rf_waveform_index,double A[3],double dt,double phase,double frequency) {
	for (int i=0; i<d->m_isochromats.count(); i++) {
		Isochromat *II=&d->m_isochromats[i];
		double pos0[3];
		pos0[0]=II->x;
...
void VmriGpuInterface::readout(double *output_real,double *output_imag,int N,double A[3],double dt,double frequency) {
...
void VmriGpuInterfacePrivate::rotateZ(double *M,double theta) {
...
void VmriGpuInterfacePrivate::rotateY(double *M,double theta) {
	double costheta=cos(theta);
	double sintheta=sin(theta);
...
void VmriGpuInterfacePrivate::decayMag(double *M,double E1,double E2) {
	M[0]*=E2;
	M[1]*=E2;
	M[2]=1-(1-M[2])*E1;
}

double VmriGpuInterfacePrivate::dot_product(double *V1,double *V2) {
	return V1[0]*V2[0]+V1[1]*V2[1]+V1[2]*V2[2];
}
void VmriGpuInterfacePrivate::apply_matrix(double *Mag,const Matrix44 &MM) {
...
Matrix44 VmriGpuInterfacePrivate::compute_RF_matrix(const QList<double> &data_real,const QList<double> &data_imag,double dt,double dtheta) {
	double T1=m_T1;
	double T2=m_T2;
